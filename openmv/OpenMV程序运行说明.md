...
1 kHz 粗识别使用最近 12 帧中的至少 8 个有效结果；10 kHz 按
`ref/main_10k_new_dataset.py` 使用 1820 张新数据集模型、最近 20 帧中的
至少 12 个有效结果，并以 1 秒为
周期做 ±0.3 倍聚类投票。10 kHz 连续 1.2 秒丢失波形时会清空历史，
避免把移动相机前后的结果混合。粗识别现在没有失败硬超时：稳定共识一旦
...
UART 上报的 `ratio_x1000` 已经是补偿后的值，STM32 端不要重复补偿。
1 kHz 模型不受影响。相机启动预热已同步为参考程序的 1000 ms，随后冻结
自动增益、自动曝光和自动白平衡。
