...
```text
E663E8855D711E0BF85F2A72FDB0E9E438D5A6CF07D588B77E1916B5DC067FF8
```
...
10 kHz 模型和补偿参数与 `ref/main_10k_new_dataset.py` 保持一致：最近 20 个有效单帧
结果中至少积累 12 帧，每 1000 ms 做一次 ±0.3 倍聚类，聚类占比至少
50% 即立即上报；连续 1200 ms 没有找到波形时清空快速共识窗口。
粗识别不再设置失败硬超时。30 秒后若尚未形成稳定共识，但本 Session
曾识别到有效轨迹，则从最多 40 个有效候选中选择最密集的一簇，以
`result=0x01` 上报；完全没有有效轨迹时继续采集，不发送失败结果。
1 kHz 仍使用 12 帧窗口、至少 8 帧规则，困难画面采用同一持续等待策略。

新 10 kHz 模型存在实测固定偏差，因此 OpenMV 在时间投票前按参考程序
执行分段补偿：原始估计 20～50 kHz 时加 1 kHz；原始估计不低于 51 kHz
时加 2 kHz；最后限制到 10～100 kHz。`COARSE_RESULT.ratio_x1000`
已经包含这项补偿，F407 不得再次补偿，否则会形成二次偏移。1 kHz 模型
不使用该补偿。
...
- 1 kHz 档：910 张，正弦波 1～10 kHz，100 Hz 步进；
- 10 kHz 档：新的单位置数据集共 1820 张，正弦波 10～100 kHz，
  1 kHz 步进；
- 实测时使用白色背景板增强绿色轨迹对比度。

### 4.2 算法

OpenMV 对 QVGA RGB565 图像进行绿色轨迹提取，定位主要波形区域，将
区域缩放为 32×32 特征，然后使用量化 PCA 和最近邻模型判断频率倍数。
...
|---|---:|---:|
| 1 kHz 扫描 | 48 | 910 |
| 10 kHz 扫描（新数据集） | 38 | 1820 |

程序不会仅使用第一帧。1 kHz 档最多保留最近 12 个有效结果且至少需要
8 个；10 kHz 档最多保留最近 20 个有效结果且至少需要 12 个。两档都在
±0.3 倍范围内聚类并取中位数，以抑制刚切换信号时的错误帧和偶发别名。
10 kHz 固定偏差补偿发生在单帧结果进入 20 帧窗口之前。

### 4.3 离线验证结果

| 模型 | 验证方式 | 主要结果 |
|---|---|---|
| 1 kHz | 固定三帧中位数 | 误差≤100 Hz：96.70%；误差≤200 Hz：100% |
| 1 kHz | 覆盖全部 910 张的滚动验证 | 误差≤100 Hz：93.08%；误差≤200 Hz：98.90% |
| 10 kHz 新模型 | 当前参考文件未附离线指标 | 需按 1820 张新数据集重新出具验证报告 |

粗识别适合给 F407 确定后续 DDS 搜索起点，不应把 10 kHz 档的粗结果
理解为已经达到 100 Hz 精调精度。旧 910 张 10 kHz 模型的验证数字不再
用于描述本次 1820 张新模型。

### 4.4 时间

当前 IDE 开图和 OLED 开启时，实际帧率取决于模型和画面复杂度。粗识别
没有失败硬超时：稳定共识形成后立即返回；30 秒后有候选但未稳定时返回
最佳候选；没有有效轨迹则持续等待。F407 每 5 秒重放一次完全相同的
`START_TASK` 用于链路恢复，但不会因等待次数进入错误态。相机启动时先让
自动控制稳定 1000 ms，随后冻结增益、曝光和白平衡，以匹配新 10 kHz
数据集的采集条件。
...
| `0x02` | 没找到有效曲线（旧版本兼容） | ACK，结束旧 Session 并自动重新识别 |
| `0x06` | 识别超时（旧版本兼容） | ACK，结束旧 Session 并自动重新识别 |
| 其他 | 见完整协议 | 非法范围返回 NACK，并自动重新识别 |

当前 OpenMV 粗识别正常只发送 `0x00` 或 `0x01`，不会再因等待时间、临时
模型载入失败或单次相机采集失败发送 `0x02/ratio=0`。模型和相机会在本机
节流重试。STM32 保持 DAC 输出并停留在 Task5 流程，用户仍可随时用三个
模式键取消当前 Session 并开始新的选择。
...
| 等待 COARSE_RESULT | 无失败硬超时；每 5 s 重放同一 START_TASK |
| 等待 DDS_TEST_RESULT | 2 s |
| COARSE_RESULT 等待重放 | 不限次数，用户可按模式键取消/重选 |
| DDS_TEST_RESULT 等待最大重放 | 3 次 |
| DDS 本地稳定等待 | 200 ms |
