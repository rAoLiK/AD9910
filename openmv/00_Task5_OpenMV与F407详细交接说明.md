...
粗识别历史，也不启动 30 秒粗识别回退计时。检测器先在固定中心范围寻找
足够高的绿色 XY 轨迹，再测量其水平包围宽度：无输入参考画面的居中细竖线
会被定位，但宽度不足，不算有效信号。宽度采用最近 7 帧中值（至少 5 帧才
判定），进入门限为画面宽度的 10% 且宽高比不小于 0.20；退出门限放宽为
7.5%/0.14，形成迟滞。滤波结果连续满足 6 次后才进入 `COARSE`，单帧刷新
缺失不会造成计数抖动。IDE 叠字 `W:xx%` 可直接用于现场核对宽度余量。
...
3. 不接外部信号按模式键，确认 OpenMV 一直显示 `WAIT SIGNAL`，不发送
   `COARSE_RESULT`；
4. 接入信号，确认 `W:xx%` 稳定越过 10%，经过中值滤波与 6 次确认后才
   显示 `COARSE` 并开始频率识别；
