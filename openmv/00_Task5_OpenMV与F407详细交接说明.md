...
```text
1560AE3534E424E98912F18B7A65EBE982E9E407979E1273BBAF94CE1B589FD1
```
...
两套模型和补偿参数分别与 `ref/main_1k_new_dataset.py`、
`ref/main_10k_new_dataset.py` 保持一致：最近 20 个有效单帧结果中至少
积累 12 帧，每 1000 ms 做一次聚类；1 kHz 使用 ±0.4 倍半径，10 kHz
使用 ±0.3 倍半径。聚类占比至少 50% 即立即上报；连续 1200 ms 没有
找到波形时清空快速共识窗口。
...
已经包含这项补偿。1 kHz 模型也按参考程序补偿：原始估计从 1.9、3.8、
5.6、7.4 kHz 起分别加 0.2、0.3、0.5、0.8 kHz，最后限制到 1～10 kHz。
F407 必须直接使用上报值，不得再次补偿。
...
- 1 kHz 档：新的单位置数据集共 1820 张，正弦波 1～10 kHz，
  100 Hz 步进；
...
| 模型 | PCA 维数 | 训练样本/参考样本 |
|---|---:|---:|
| 1 kHz 扫描（新数据集） | 32 | 1820 |
| 10 kHz 扫描（新数据集） | 38 | 1820 |

程序不会仅使用第一帧。两档都最多保留最近 20 个有效结果且至少需要
12 个；1 kHz 在 ±0.4 倍、10 kHz 在 ±0.3 倍范围内聚类并取中位数，
以抑制刚切换信号时的错误帧和偶发别名。两档固定偏差补偿均发生在单帧
结果进入 20 帧窗口之前。

### 4.3 离线验证结果

当前两份 1820 张新数据集参考程序均未附可复核的离线指标，应使用各自
数据集重新出具验证报告。旧 910 张模型的验证数字不再用于描述当前模型。
