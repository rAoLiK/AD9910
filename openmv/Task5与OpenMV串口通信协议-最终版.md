...
当 `saw_freq_hz == 1000` 时使用 1 kHz 模型自己的实测补偿：原始估计
从 1.9、3.8、5.6、7.4 kHz 起分别增加 0.2、0.3、0.5、0.8 kHz，随后
限制到 1～10 kHz。该补偿同样已经包含在 `ratio_x1000` 中，STM32 不得
再次补偿。以上变化均不改变帧格式、字段长度、CRC 或 ACK 行为。
