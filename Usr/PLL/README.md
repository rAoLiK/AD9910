...
- 中高频捕获时，相位误差经环形低通和 PI 环路转成 DDS 频率微调；
  低频使用 POW 直接校相并学习 FTW 偏差，因此整体是混合环路。
- 参考输入达到约 59 kHz 后，捕获过程仍使用原 PI 环路；锁定条件累计
  达到 150 ms 后固定为单一 FTW，停止相邻 FTW 的分数调制。剩余亚 FTW
  相位漂移由带 0.2° 死区、单步不超过 0.08° 的 POW 微调维持，从而避免
  60~100 kHz 锁定后的可见频率抽动。退出锁定或确认输入变频后自动恢复
  原捕获环路。
- 高频频率/相位仍有效且滤波误差已进入 30° 近锁区时，1 s acquire
  watchdog 只延长捕获窗口，不再清空整个控制器；真实变频的 600 ms
  `CHANGE_PENDING` 超时仍会重搜。
...
或参考频率已经下降到约 57 kHz 以下。完整参数含义和调参顺序见工程根
目录的 [`README.md`](../../README.md)。
...
4. 配置 MUL=2，确认 DDS 频率为输入两倍；观察稳定的二倍频莉萨如图。
   在 60 kHz、70 kHz 和 100 kHz 输入下等待 `frequency_hold_mode=1`，确认锁定后
